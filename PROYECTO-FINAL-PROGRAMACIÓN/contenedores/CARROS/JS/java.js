...
// EMPEZAMOS A CALCULAR EL RADIO DE LA PISTA
const trackRadius = 225;
const trackWidth = 45;
const innerTrackRadius = trackRadius - trackWidth;
const outerTrackRadius = trackRadius + trackWidth;

// CALCULAMOS LOS ÁNGULOS DE LAS ISLAS
const arcAngle1 = (1 / 3) * Math.PI; // 60 GRADOS

const deltaY = Math.sin(arcAngle1) * innerTrackRadius;
const arcAngle2 = Math.asin(deltaY / outerTrackRadius);

// SE CALCULAN LA DISTANCIA ENTRE LOS CENTROS
const arcCenterX =
  (Math.cos(arcAngle1) * innerTrackRadius +
    Math.cos(arcAngle2) * outerTrackRadius) /
  2;

  // SE CALCULAN LOS DEMÁS ÁNGULOS
...
// SE INCIA THREE.JS
// SE INDICA LOS PARÁMETROS DE LA CÁMARA
const aspectRatio = window.innerWidth / window.innerHeight;
const cameraWidth = 970; // SE INDICA EL TAMAÑO DEL MAPA EN LA PANTALLA
...
// SE INDICA HACIA DONDE DEBE ESTAR VIENDO LA CÁMARA
...
renderMap(cameraWidth, cameraHeight * 2); // EL MAPA ES MÁS GRANDE PORQUE LO VEMOS DESDE UN ÁNGULO

// SE AÑADEN LAS LUCES DEL ESCENARIO
const ambientLight = new THREE.AmbientLight(0xffffff, 0.6); // SE INDICA LA INTENSIDAD
scene.add(ambientLight);

const dirLight = new THREE.DirectionalLight(0xffffff, 0.6);
dirLight.position.set(100, -300, 400); // SE INDICA DESDE DONDE VIENE LA LUZ EN LOS 3 EJES (X Y X)
scene.add(dirLight);
...
// SE RENDERIZA PARA QUE SE AJUSTE AL TAMAÑO DEL HTML
...
// SE CREAN LAS LÍNEAS DE LA PISTA
...
function renderMap(mapWidth, mapHeight) {
  ...
  // Geometría extruida con bordillos
...
// SE CREA LA TEXTURA DE LA CABINA DEL CARRO
function getCarFrontTexture() {
  const canvas = document.createElement("canvas");
  canvas.width = 64;
  canvas.height = 32;
  const context = canvas.getContext("2d"); // SE CREA EL LIENZO

  context.fillStyle = "#ffffff"; // SE LE DA COLOR
  context.fillRect(0, 0, 64, 32); // SE LE DAN SUS DIMENSIONES

  // CREAMOS UN RECTÁNGULO MÁS PEQUEÑO QUE SIMULARÁ LAS VENTAVAS DE LA CABINA
...
function Car() {
  ...
  // SE AGREGA LA ESTRUCTURA DEL CARRO
  ...
  // SE INDICAN LAS ROTACIONES DE LAS TEXTURAS DE LA CABINA
  ...
  // AQUÍ SE CREAN LOS NEUMÁTICOS DELANTEROS Y TRASEROS
...
// BOTONES PARA JUGAR CON EL MOUSE
...
function animation(timestamp) {
  ...
  // AGREGAR UN VAEHÍCULO CADA 5 VUELTAS
...
// SE INDICA COMO DEBE MOVERSE EL CARRO DEL JUGADOR 
...
function getPlayerSpeed() {
 ...
 //SE AGREGAN LOS OTROS VEHÍCULOS
function addVehicle() {
  const vehicleTypes = ["car", "truck"];

  const type = pickRandom(vehicleTypes);
  const speed = getVehicleSpeed(type);
  const clockwise = Math.random() >= 0.5; // AQUÍ EL PROGRAMA SELECCIONA DE MANERA ALEATOIA HACIA DONDE SE MOVERÁ EL SIGUIENTE VEHÍCULO
...
function hitDetection() {
  ...
  // SE INDICAN LAS ZONAS DE IMPACTO PARA CUANDO LOS VEHÍCULOS CHOQUEN Y SE DETENGA EL JUEGO
